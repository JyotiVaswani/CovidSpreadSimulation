...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        resume();                   // timer resumed before sorting is called
        U t2= function.apply(t1);   // call to insertion sort
        pauseAndLap();              // timer is paused and lap is incremented after sorting is completed
        if (postFunction!=null)
        { postFunction.accept(t2);}
        };
        double mean_time = meanLapTime();
        System.out.println("Average mean time taken: "+mean_time);
        return mean_time;
    }
    ...
    public double meanLapTime() {
       ...
       // System.out.println("laps "+ laps);
        double milsec=toMillisecs(ticks);
    ...
    private static double toMillisecs(long ticks) {
        double msec = ticks/Math.pow(10,6);
