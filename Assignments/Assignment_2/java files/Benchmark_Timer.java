...
public class Benchmark_Timer<T> implements Benchmark<T> {
    ...
    public static Integer[] getRandomArray(int n)
    {
        ...
        for (int i = 0; i < random_array.length; i++) {
            random_array[i] = new Random().nextInt(10000);
    ...
    public static void main (String args[]) throws IOException {
     /*
        This loop is used to implement doubling on number of elements in array (n) which is further used for timing the Insertionsort over the array
    */

        for (int n = 1000; n < 100000;) {
            n = n*2;
            System.out.println();
            System.out.println();
            System.out.println("Number of elements in array:  " +n);
            Benchmark_Timer.arrayInput(n);
    ...
    public static void arrayInput(int n){
        ...
        /*
        Below run() will time the Insertionsort over random array
         */
        Integer set1[] = Benchmark_Timer.getRandomArray(n);  // generating array of random Integers
        Consumer<Integer[]> run_method = numbs -> is.sort(numbs, 0,numbs.length);
        Benchmark_Timer<Integer[]> bmt = new Benchmark_Timer<Integer[]>
                ("Sorting_Array_of_Integers", null,run_method, null);

        System.out.println("Random array: ");
        bmt.run(set1,10);

       /*
        Below run() will time the Insertionsort over partially sorted array (first-half of array is sorted )
         */
        Arrays.sort(set1,0,((set1.length)/2));
        System.out.println("Partially sorted array: ");
        bmt.run(set1,10);


        /*
        Below run() will time the Insertionsort over already sorted array
         */
        Arrays.sort(set1);
        ...
        /*
        Below run() will time the Insertionsort over reverse sorted array
         */
        Arrays.sort(set1, Collections.reverseOrder());
